...
public interface AvaliacaoRepository extends JpaRepository <AvaliacaoModel, Long> {
    @Query(value = """
            SELECT data, abdominais, altura, burpees, cooper, flexoes, forca_isometrica_maos, peso, prancha, rm_terra, teste_de_lunge, atleta_id
            FROM (
                SELECT\s
                    data, abdominais, altura, burpees, cooper, flexoes, forca_isometrica_maos, peso, prancha, rm_terra, teste_de_lunge, atleta_id,
